...
private:
    ...
    // Physical constants
    const double DRAG_COEFFICIENT = 0.32; // Aerodynamic drag coefficient for Sedan
    const double ROLLING_RESISTANCE = 0.015;// Rolling resistance coefficient for typical tires
    const double VEHICLE_MASS = 1450.0;
    const double WHEEL_RADIUS = 0.3; // meters
    const double FINAL_DRIVE_RATIO = 3.7;  // Final drive ratio for the vehicle the engine must rotate 3.7 times for one wheel rotation
    const double MAX_ENGINE_TORQUE = 250.0; // Nm, maximum torque of the engine at peak performance
    ...
    chrono::steady_clock::time_point last_update;// Last update time for simulation step

public:
    VehicleSimulator() : // Constructor initializes the simulator
        can_socket(-1),
        running(true), 
        engine_on(false), 
        speed_ms(0.0), 
    ...
    void initializeCAN() {// Initialize the CAN socket for communication
